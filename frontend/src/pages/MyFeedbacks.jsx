...
export default function MyFeedbacks() {
  ...
  const deleteAll = async () => {
    ...
    if (!window.confirm("Delete ALL your feedbacks?")) return;
    try {
      await Promise.all(list.map((fb) => API.delete(`/feedback/${fb._id}`)));
      // After deleting all on current page, reload page 1
      await fetchPage(1);
    } catch (err) {
      alert("Error deleting all feedbacks.");
    }
  };
    ...
    <>
      ...
      <div className="home-container">
        ...
        <div className="delete-all-wrapper">
          <button className="danger-all" onClick={deleteAll}>
          ...
          <ul className="feedback-list">
            {list.map((fb) =>
              editingId === fb._id ? (
                <li key={fb._id} className="feedback-card">
                  <div>
                    <strong>Course:</strong> {fb.course?.title}
                  </div>
